...
- **UE 균형 (이슈 #29)** — `engine/ue.py`. 하루 단위 반복 균형을 찾고, 그 배정을 DES 에
  그대로 넣는다 (정차마다 체류시간이 같다는 것을 테스트로 고정). `docs/UE_equilibrium.md`.
- **S0** — §4 의 `wait_min` 을 보고 충전소를 고른다. DES 는 그 결정을 받아서 돌린다.

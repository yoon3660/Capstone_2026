...
차로수는 1이라고 적어둔 링크**가 있었다. 경주IC 북쪽 78.202~81.930 km 에 5개가
...
포함 여부를 따졌다. 대표점이 78.202~81.930 구간 밖이라 이 구간은 검증을 한 번도
...
- 위 수정으로 차로 프로파일을 **다시 생성**해야 한다 (표준노드링크 SHP 필요).
  생성 후 1차로 구간이 사라졌는지, 메운 구간 목록이 타당한지, 실측 교통량
  교차검증을 통과하는지 확인할 것.
